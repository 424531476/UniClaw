...
async def cmd_cost(_args: str, config: AppConfig) -> bool:
    ...
    await info(f"  输入 tokens:   {in_tokens:>12,}", config)
    await info(f"  输出 tokens:   {out_tokens:>12,}", config)
    await info(f"  总 tokens:     {total_tokens:>12,}", config)
    await info(f"  API 调用:      {api_calls:>12,}", config)
    await info(f"  工具调用:      {tool_calls:>12,}", config)

    # 按模型计费
    if by_model:
        total_cost = 0.0
        await info(f"\n按模型计费:\n", config)
        await info(f"  {'模型':<28} {'输入':>10} {'输出':>10} {'调用':>6} {'费用':>10}", config)
        await info(f"  {'─'*28} {'─'*10} {'─'*10} {'─'*6} {'─'*10}", config)
        for model_name, m in sorted(by_model.items()):
            ...
            , config)
        await info(f"  {'─'*28} {'─'*10} {'─'*10} {'─'*6} {'─'*10}", config)
        await info(f"  {'合计':<28} {in_tokens:>10,} {out_tokens:>10,} "
             f"{api_calls:>6} ${total_cost:>9.4f}", config)
    else:
        # 兼容旧数据(无 by_model)
        await info(f"  (旧数据无费用记录,升级后的新调用将自动计费)", config)

    # 每日统计
    if daily:
        await info(f"\n最近 7 天:\n", config)
        await info(f"  {'日期':<12} {'输入':>10} {'输出':>10} {'合计':>10} {'调用':>6} {'费用':>10}", config)
        await info(f"  {'─'*12} {'─'*10} {'─'*10} {'─'*10} {'─'*6} {'─'*10}", config)
