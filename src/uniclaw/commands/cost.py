...
def cmd_cost(_args: str, config: AppConfig) -> bool:
    ...
    info(f"\n费用统计\n", config)
    info("─" * 50, config)
    info(f"  输入 tokens:   {in_tokens:>12,}", config)
    info(f"  输出 tokens:   {out_tokens:>12,}", config)
    info(f"  总 tokens:     {total_tokens:>12,}", config)
    info(f"  API 调用:      {api_calls:>12,}", config)
    info(f"  工具调用:      {tool_calls:>12,}", config)

    # 按模型计费
    if by_model:
        total_cost = 0.0
        info(f"\n按模型计费:\n", config)
        info(f"  {'模型':<28} {'输入':>10} {'输出':>10} {'调用':>6} {'费用':>10}", config)
        info(f"  {'─'*28} {'─'*10} {'─'*10} {'─'*6} {'─'*10}", config)
        ...
        # 兼容旧数据(无 by_model)
        info(f"  (旧数据无费用记录,升级后的新调用将自动计费)", config)

    # 每日统计
    if daily:
        info(f"\n最近 7 天:\n", config)
        info(f"  {'日期':<12} {'输入':>10} {'输出':>10} {'合计':>10} {'调用':>6} {'费用':>10}", config)
        info(f"  {'─'*12} {'─'*10} {'─'*10} {'─'*10} {'─'*6} {'─'*10}", config)
