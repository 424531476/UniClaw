...
async def cmd_goal(args: str, config: AppConfig) -> bool:
    ...
    if goal_mgr is None:
        await warn("当前任务没有 GoalManager(仅 root 任务支持)", config)
        return True
    ...
    if arg.lower() == "clear":
        if not goal_mgr.active:
            await warn("当前没有设置目标", config)
        else:
            goal_mgr.clear_goal()
            await ok("目标已清除", config)
        return True

    # /goal status
    if arg.lower() == "status":
        await info(goal_mgr.get_status(), config)
        return True

    # /goal <描述> — 设置目标
    if arg:
        goal_mgr.set_goal(arg)
        await ok(f"目标已设置: {arg}", config)
        await info("agent 将在每次尝试停止时评估目标是否达成", config)
        await info(f"最大重入次数: {goal_mgr.max_reentry}", config)
        return True

    # 无参数: 显示状态
    await info(goal_mgr.get_status(), config)
    if not goal_mgr.active:
        await info("  使用 /goal <目标描述> 设置目标", config)
    return True
